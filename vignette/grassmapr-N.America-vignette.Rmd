...
We will combine the crossover temperature model with data on land cover and climate to produce map of C~3~ and C~4~ plants in North America. Then, we will apply isotopic endmember values to produce a spatially continuous representation of the $\delta$^13^C distribution across the continent. These layers are useful for understanding grass biogeography and for studies seeking to identify the movement of animals (e.g., Hobson, 1999; Powell et al., 2012)
...
tmp <- temp_NA        # Mean Monthly Temperature (deg. C)
pre <- precip_NA      # Mean Monthly Precipitation (mm)
ndvi <- ndvi_NA       # Mean Monthly NDVI ("greenness")
woody <- woody_NA     # Woody Cover (%)
cropC3 <- cropC3_NA   # C3 Crop Cover (%)
cropC4 <- cropC4_NA   # C4 Crop Cover (%)
...
# Growing season precipitation mask (>= 25 mm)
  precip_mask <- mask_grids(pre, precip_threshold)
# Growing season temperature mask (>= 5 deg. C)
  GS_temp_mask <- mask_grids(tmp, gs_threshold)
# C4 temperature mask (>= 22 deg. C)
  C4_temp_mask <- mask_grids(tmp, temp_threshold)
...
# Create raster stack of other (non-grassy) vegetation layers
  veg_layers <- stack(woody, cropC3, cropC4)
...
# Calculate weighted standard deviation of mean d13C values
  ...
  plot(d13C_iso, main = expression(Plant~{delta}^13*C~"\211"),
    xlab = "", ylab = "", zlim = c(-27,-12))  # Mac users: substitute "\u2030"
  plot(d13C_iso_std, main = expression(StDev~{delta}^13*C~"\211"),
    xlab = "", ylab = "", zlim = c(1.0, 2.4)) # Mac users: substitute "\u2030"
